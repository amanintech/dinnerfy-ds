...
@app.post("/recipe/{RecipeID}")
def get_recipe(RecipeID: int, servings: int):
    try:
        recipe = df.iloc[RecipeID - 1]  # Access the row by recipe_id
        recipe_data = {
        ...
        raise HTTPException(status_code=403, detail="RecipeID or servings not found")
